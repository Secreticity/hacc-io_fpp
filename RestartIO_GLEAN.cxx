...
//JIWOO
    ...
    //strncat(pathname, buffer, strlen(buffer));
    //printf("pathname: %s\n", pathname);
    ...
    snprintf (m_partFileName, m_partFileNameLen, "%s-Part%08d-of-%08d-%d.data",
              m_basePathName, m_partitionID, m_totPartitions, rand());
...
int RestartIO_GLEAN :: Close (void)
{
    ...
    if (0 == m_globalCommRank)
    {
        ...
        tot_data_size = tot_data_size / (1024.0 * 1024.0);
        switch (m_mode)
        {
            case READ_RESTART:
                cout << " READ Restart Perf: " << bw << " BW[MB/s] " << tot_data_size << " MBytes " << agg_max_time << " MaxTime[sec] " <<  endl;
                break;
            case WRITE_CHECKPOINT:
                cout << " WRITE Checkpoint Perf: " << bw <<" BW[MB/s] " << tot_data_size << " MBytes " << agg_max_time << " MaxTime[sec] " <<  endl;
                break;
        }
